...
ig.module('game.keyboard').requires('impact.font').defines(function() {
    ig.Keyboard = ig.Class.extend({
        ...
        init: function(callback, layout) {
            ...
            if (ig.ua.mobile) {
                this.keyboard = ig.$new('canvas');
                this.keyboard.width = this.background.width;
                this.keyboard.height = this.background.height;
                this.keyboardContext = this.keyboard.getContext('2d');
            }
        ...
        drawFull: function() {
            ...
            ctx.drawImage(this.keyboard, this.x, this.y);
            ...
            if (this.hoverKey) {
                ...
                var a = this.unsetHoverKeyTimer ? this.unsetHoverKeyTimer.delta().map(0, 0.25, 1, 0) : 1;
...
ig.module('game.main').requires('impact.game', 'impact.font', 'game.menus.about', 'game.menus.game-over', 'game.menus.settings', 'game.menus.title', 'game.entities.enemy-missle', 'game.entities.enemy-mine', 'game.entities.enemy-destroyer', 'game.entities.enemy-oppressor', 'game.entities.player', 'game.keyboard', 'game.xhr', 'game.ease', 'plugins.silent-loader', 'plugins.rise-loader', 'game.document-scanner', 'game.words.en').defines(function() {
    ...
    ZType = ig.Game.extend({
        ...
        purchaseRemoveAds: function() {
            ...
            this.iap.getProducts(['removeAds'],
            function(error, products) {
                if (error) {
                    ig.game.waitingForItunes = false;
                    ig.game.setTitle();
                } else if (products.length) {
                    products[0].purchase(1,
    ...
    if (window.ZTypeDocumentMode) {
        ...
        if (!ig.ua.mobile) {
            ig.doc.playScanAnimation(function() {
                if (!ig.ua.mobile) { (adsbygoogle = window.adsbygoogle || []).push({});
                }
            });
        }
    }
